...
echo `echo \$GITHUB_REF_NAME`;
echo `echo \$GITHUB_SHA`;
echo `echo git diff --name-only \$GITHUB_HEAD_REF \$GITHUB_BASE_REF`;

$modifiedFilescommand = <<<COMMAND
#!/usr/bin/env bash
echo "last commit hash:"
echo "$(git rev-parse HEAD)"
git diff --name-only \$GITHUB_HEAD_REF \$GITHUB_BASE_REF | while read -r filename; do
  echo "filename:"
  echo "\$filename"
  echo "modified:"
